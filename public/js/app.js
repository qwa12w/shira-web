...
// [النسخة المستقرة - التصميم ثلاثي الأبعاد]
// ==========================================
      ...
      c.innerHTML = 
        '<div class="top-bar" style="display:flex;justify-content:space-between;align-items:center;padding:1rem 1.2rem;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);border-radius:0 0 24px 24px;margin:-1rem -1rem 1rem -1rem;box-shadow:0 8px 32px rgba(102,126,234,0.4);">' +
          '<div style="display:flex;align-items:center;gap:0.75rem;cursor:pointer;" onclick="showProfileEditor()">' +
            '<div style="width:42px;height:42px;background:rgba(255,255,255,0.25);border-radius:50%;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:bold;font-size:1.1rem;border:2px solid rgba(255,255,255,0.5);">' + initials + '</div>' +
...
function showActivationModal(userId) {
  ...
  modal.innerHTML = 
    '<div style="background:#fff;padding:2rem;border-radius:20px;width:90%;max-width:350px;text-align:center;box-shadow:0 10px 30px rgba(0,0,0,0.2);">' +
      '<h3 style="margin:0 0 1rem 0;">⏳ مدة التفعيل</h3>' +
      '<select id="activation-months" style="width:100%;padding:0.8rem;margin-bottom:1rem;border:2px solid #e2e8f0;border-radius:12px;font-size:1rem;">' +
        '<option value="1">شهر</option><option value="2">شهرين</option><option value="3">3 أشهر</option><option value="6">6 أشهر</option><option value="12">سنة</option>' +
      '</select>' +
      '<div style="display:flex;gap:0.5rem;">' +
        '<button onclick="confirmActivation(\'' + userId + '\')" style="flex:1;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:#fff;border:none;padding:0.8rem;border-radius:12px;cursor:pointer;font-weight:600;">✅ تأكيد</button>' +
...
function confirmActivation(userId) {
  ...
  var expiryDate = new Date(); expiryDate.setMonth(expiryDate.getMonth() + months);
  var client = window.supabaseClient;
  client.from('profiles').update({ status: 'نشط', subscription_expiry: expiryDate.toISOString() }).eq('id', userId).then(function(res) {
    ...
    else { alert('✅ تم التفعيل'); loadUsersTable(); }
  });
}
  ...
  c.from('profiles').select('*').order('created_at', { ascending: false }).then(function(res) {
    ...
    if (res.data) {
      res.data.forEach(function(u) {
        ...
        var actions = '';
        if (u.status === 'قيد المراجعة') actions += '<button class="btn-action" onclick="showActivationModal(\'' + u.id + '\')">⏳ تفعيل</button>';
        else if (u.status === 'نشط') actions += '<button class="btn-action" onclick="changeStatus(\'' + u.id + '\',\'محظور\')">🚫 حظر</button>';
        else if (u.status === 'محظور') actions += '<button class="btn-action" onclick="changeStatus(\'' + u.id + '\',\'نشط\')">✅ تفعيل</button>';
...
function changeStatus(id, st) { var c = window.supabaseClient; if(!c)return; c.from('profiles').update({status:st}).eq('id',id).then(function(){loadUsersTable();}); }
...
function initOrderMap(lat, lng) {
  var mapEl = document.getElementById('order-map'); if (!mapEl || typeof L === 'undefined') return;
  mapEl.innerHTML = '';
